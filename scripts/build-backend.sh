...
# is renamed to the Linux Tauri target triple. Keep the two scripts in sync.
set -euo pipefail
...
echo "==> Locating bundled skills (skills/)..."
SKILLS_BUNDLE="$ROOT/skills"
if [ ! -d "$SKILLS_BUNDLE" ]; then
    echo "skills/ not found — cannot bundle skills into backend." >&2
    exit 1
fi
...
pyinstaller run.py \
    ...
    --add-data "${SKILLS_BUNDLE}:bundled_skills"
