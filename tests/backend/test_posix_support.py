...
def test_resolve_active_shell_is_posix_on_non_windows(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Every saved preference collapses to "posix" — WSL/PowerShell are
    # Windows-only concepts.
    assert main.resolve_active_shell("auto") == "posix"
    assert main.resolve_active_shell("wsl") == "posix"
    assert main.resolve_active_shell("powershell") == "posix"
...
def test_resolve_active_shell_honours_forced_modes_on_windows(
    ...
    assert main.resolve_active_shell("wsl") == "wsl"
    assert main.resolve_active_shell("powershell") == "powershell"
...
@pytest.mark.asyncio
async def test_bash_tool_posix_execute_routes_to_native(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tool = BashTool(user_name="user", user_home="/home/user", shell="posix")
    seen: dict[str, str] = {}

    async def fake_exec_posix(command: str) -> str:
        seen["command"] = command
        return "native-ok"

    monkeypatch.setattr(tool, "_exec_posix", fake_exec_posix)
