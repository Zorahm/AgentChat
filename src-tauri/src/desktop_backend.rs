//! Desktop-only Python backend sidecar management.
//!
//! The desktop app spawns and supervises the bundled `agentchat-backend`
//! sidecar on 127.0.0.1:8787. None of this exists on mobile (Android/iOS),
//! where the app is a thin client talking to a remote backend the user
//! configures in-app — so this whole module is gated `#[cfg(desktop)]` by its
...
#[tauri::command]
pub fn restart_backend(state: tauri::State<'_, BackendProcess>) -> Result<(), String> {
...
/// Decide which backend serves 8787 and return the child we own (if any).
///
/// Dev (debug build): reuse a hand-run uvicorn if one is already healthy, so a
/// developer's own backend is never killed; otherwise spawn.
///
/// Release build: insist on a backend reporting OUR version. A leftover sidecar
/// from a previous version keeps serving 8787 after an update (the new sidecar
/// can't bind a second time, and an older generically-named `backend.exe` slips
/// past taskkill-by-image), so the new UI would talk to a stale backend missing
/// newer settings fields. Force-clear the port and run the freshly-bundled
/// backend instead. Blocks until the chosen backend is healthy.
pub fn start_managed_backend(expected_version: &str) -> Option<Child> {
    let probe_client = reqwest::blocking::Client::new();

    let backend_child: Option<Child> = if cfg!(debug_assertions) {
        ...
        match find_sidecar_exe() {
            Some(sidecar) => {
                ...
                if running.as_deref() == Some(expected_version) {
...
/// On app exit, tear down the whole sidecar tree first: child.kill() only reaps
/// the PyInstaller bootloader and would orphan the interpreter it spawned,
/// leaking a backend on 8787.
